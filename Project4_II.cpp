...
bool hasValidSyntax(const char orders[])
{
	...
	while (k != strlen(orders))
	{
		...
		char state[3] = { orders[k-2], orders[k-1], '\0' };
...
int main()
{
	...
	assert(countCases("Ca55+ca006-Tx66+ny05-", '+', cases) == 0 && cases == 121);
	assert(countCases("Ca55+ca006-Tx66+ny05-", '-', cases) == 0 && cases == 11);
	assert(countCases("Ca000+ca006-Tx66+ny05-", '-', cases) == 2 && cases == 11);
	assert(countCases("Ca55+ca006-Tx00+ny05-", '-', cases) == 2 && cases == 11);
	assert(countCases("Ca55+ca006-Tx66+ny05-", '@', cases) == 3 && cases == 11);
	assert(countCases("Ca55+ca006-Tx+ny05-", '-', cases) == 1 && cases == 11);
	assert(countCases("Ca55+ca006-Tx66#ny05-", '-', cases) == 1 && cases == 11);
	assert(countCases("Ca55+ca006-Tx@6+ny05-", '-', cases) == 1 && cases == 11);
	assert(countCases("Ca55+ca006-YY6+ny05-", '-', cases) == 1 && cases == 11);
	assert(countCases("Ca55+ca0-YY6+ny05-", '-', cases) == 1 && cases == 11);
	assert(countCases("Ca55+ca006-Tx@6+ny00-", '-', cases) == 1 && cases == 11);
	assert(countCases("", '-', cases) == 0 && cases == 0);
	cases = 99;
	assert(countCases("Ms5+Ca66--", '-', cases) == 1 && cases == 99);
	cases = 98;
	assert(countCases("><", '-', cases) == 1 && cases == 98);
